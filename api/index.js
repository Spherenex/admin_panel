...
// api/index.js - Vercel serverless function with comprehensive health check
const Razorpay = require('razorpay');
const crypto = require('crypto');

// Helper function to validate Razorpay configuration
const validateRazorpayConfig = () => {
  const keyId = process.env.RAZORPAY_KEY_ID || 'rzp_test_psQiRu5RCF99Dp';
  const keySecret = process.env.RAZORPAY_KEY_SECRET || 'MLb1hejwBSaeg9ysJjO24O0u';
...
module.exports = async (req, res) => {
  ...
  // Health check for GET request
  if (req.method === 'GET') {
    try {
      // Initialize Razorpay with configured credentials
      const razorpay = new Razorpay({
        key_id: process.env.RAZORPAY_KEY_ID || 'rzp_test_psQiRu5RCF99Dp',
        key_secret: process.env.RAZORPAY_KEY_SECRET || 'MLb1hejwBSaeg9ysJjO24O0u',
      });

      // Attempt a simple order fetch to validate Razorpay connection
      let ordersCheck = null;
      try {
        ordersCheck = await razorpay.orders.all({ count: 1 });
      } catch (orderError) {
        console.error('Razorpay orders fetch failed:', orderError);
      }

      // Prepare detailed health response
      res.status(200).json({ 
        status: 'ok',
        version: '1.0.0',
        timestamp: new Date().toISOString(),
