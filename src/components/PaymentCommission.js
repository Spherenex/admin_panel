...
//   ? 'https://admin-panel-b7ji.vercel.app' // Replace with your actual deployed backend URL
//   : 'http://localhost:3000';
...
//         try {
//           // Call payment transfer API with timeout
//           const paymentResponse = await fetch(`${API_URL}/api/vendor-transfer`, {
//             method: 'POST',
//             headers: {
//               'Content-Type': 'application/json',
//             },
...
//           const itemsWithProfit = allItems.map(item => {
//             // Profit calculation: Vendor Price - Base Price
//             const profitPerUnit = item.price - item.vendorPrice;
//             const totalProfit = profitPerUnit * item.quantity;
...
//   // Process vendor items for display with corrected field mappings
//   const processedVendorItems = vendorItems.map(item => {
//     // Map the fields from the JSON structure correctly
//     return {
//       ...item,
//       // Use the correct field mappings from the order JSON
//       totalVendorPrice: (item.vendorPrice || 0) * (item.quantity || 0)
//     };
//   });

//   // Get sorted and filtered data for display
//   const displayedVendorItems = sortItems(processedVendorItems);
//   const displayedVendorOrders = sortOrders(filterOrdersByDate(vendorOrders));
...
// // Main API URL - use production URL even in development if running locally
// const API_URL = isLocalDevelopment 
//   ? 'http://localhost:3000'  // Local development server
//   : 'https://admin-panel-mu-sepia.vercel.app'; // Production server

// // Fallback API URL (used if the main one fails)
// const FALLBACK_API_URL = 'https://admin-panel-mu-sepia.vercel.app';

// // Define API endpoints with multiple possible paths
// const API_ENDPOINTS = {
//   health: `${API_URL}/api/health`,
//   // Start with the standard API path
//   vendorTransfer: `${API_URL}/api/vendor-transfer`
// };

// // Possible alternative API paths to try if the main one fails
// const ALTERNATIVE_API_PATHS = [
//   '/api/vendor-transfer',
//   '/vendor-transfer',
//   '/api/payment/vendor-transfer',
//   '/payment/vendor-transfer',
//   '/api/payments/vendor-transfer'
// ];
...
//           response = await fetch(`${FALLBACK_API_URL}/api/health`, {
//             method: 'GET',
//             signal: AbortSignal.timeout(3000)
//           });
//         }
...
//           // Filter payments by vendor and set paid status
//           // IMPORTANT CHANGE: Now we check for explicitly completed payments with status 'completed'
//           Object.values(payments).forEach(payment => {
...
//   // MODIFIED: processPayment function with mock API implementation for development
//   const processPayment = async (vendorDetails, paymentDetails) => {
//     if (!currentPaymentItem) return false;

//     const itemId = currentPaymentItem.id;
//     const MAX_RETRIES = 3; // Increased retries to try more alternative paths
...
//       console.log("Payment API endpoints:", API_ENDPOINTS);

//       // DEVELOPMENT MODE: Use mock payment API if in local development
//       if (isLocalDevelopment) {
//         console.log("Using mock payment API response for development");

//         // Simulate API call delay
//         await new Promise(resolve => setTimeout(resolve, 1500));

//         // Generate a mock payment key
//         const paymentKey = `payment_mock_${Date.now()}`;

//         // Store mock payment data in Firebase with COMPLETED status
//         const paymentRef = ref(db, `payments/${paymentKey}`);
//         await set(paymentRef, {
//           paymentId: paymentKey,
//           vendorId: selectedVendor.id,
...
//         // Update paid items state immediately
//         setPaidItems(prev => ({
//           ...prev,
//           [itemId]: true
//         }));
...
//         setNotification({
//           message: `DEVELOPMENT MODE: Mock payment of ${formatCurrency(currentPaymentItem.totalVendorPrice)} processed`,
//           details: 'This is a simulated payment for development purposes.',
//           type: 'success',
//           icon: <CheckCircle size={20} />
//         });
...
//       // PRODUCTION MODE: Continue with real API calls
//       // Prepare data for payment transfer API
//       const paymentApiData = {
//         vendor_id: selectedVendor.id,
//         amount: currentPaymentItem.totalVendorPrice,
...
//       console.log("Initiating payment transfer:", paymentApiData);
//       console.log("Initial API endpoint being used:", API_ENDPOINTS.vendorTransfer);
...
//       let success = false;
//       let error = null;
//       let currentEndpoint = API_ENDPOINTS.vendorTransfer;
//       let fallbackMode = false;

//       // Attempt payment with retries, trying different API paths
//       while (retryAttempt <= MAX_RETRIES && !success) {
//         try {
//           console.log(`Attempt ${retryAttempt + 1}/${MAX_RETRIES + 1} using endpoint: ${currentEndpoint}`);

//           // Call payment transfer API with timeout
//           const paymentResponse = await fetch(currentEndpoint, {
//             method: 'POST',
//             headers: {
//               'Content-Type': 'application/json',
//             },
...
//           // Check if payment was successful
//           if (paymentResult.status === 1) {
//             // IMPORTANT CHANGE: Set payment status to 'completed' to ensure it's properly tracked
//             // Generate a unique key for this payment
//             const paymentKey = `payment_${Date.now()}`;

//             // Store payment data in Firebase with complete details and COMPLETED status
//             const paymentRef = ref(db, `payments/${paymentKey}`);
//             await set(paymentRef, {
//               paymentId: paymentKey,
//               vendorId: selectedVendor.id,
...
//             // Create a lookup by item ID for quick status checks with completed status
//             await set(ref(db, `itemPayments/${itemId}`), {
//               paymentId: paymentKey,
//               status: 'completed', // Changed from 'initiated' to 'completed'
//               merchant_ref_id: paymentResult.data.merchant_ref_id,
...
//             // Show success notification with more details
//             setNotification({
//               message: `Payment of ${formatCurrency(currentPaymentItem.totalVendorPrice)} to ${vendorDetails.name} completed successfully!`,
//               details: `Reference ID: ${paymentResult.data.merchant_ref_id}`,
//               type: 'success',
//               icon: <CheckCircle size={20} />
//             });
...
//           console.error(`Payment attempt ${retryAttempt} error:`, attemptError);
...
//             // If we're encountering errors, let's try alternative API paths
//             if (retryAttempt <= ALTERNATIVE_API_PATHS.length) {
//               // Try the next alternative path
//               const altPath = ALTERNATIVE_API_PATHS[retryAttempt - 1];

//               // If we haven't switched to fallback mode yet and we're getting errors
//               if (!fallbackMode && (attemptError.message.includes('Failed to fetch') || 
//                   attemptError.message.includes('network error') ||
//                   attemptError.message.includes('404'))) {
//                 // Switch to production URL for subsequent attempts
//                 fallbackMode = true;
//                 console.log("Switching to fallback production URL for subsequent attempts");
//               }

//               // Determine which base URL to use
//               const baseUrl = fallbackMode ? FALLBACK_API_URL : API_URL;
//               currentEndpoint = `${baseUrl}${altPath}`;

//               console.log(`Trying alternative API path: ${currentEndpoint}`);
...
//           // If we've exhausted all retries or paths, break the loop
//           break;
//         }
//       }

//       // If we get here with no success, throw the last error with improved details
//       if (!success && error) {
//         console.error("All payment attempts failed. Last endpoint tried:", currentEndpoint);
//         throw error;
//       }
...
//         errorMessage = 'Payment request timed out. The server might be busy or not responding.';
//       } else if (error.message.includes('Failed to fetch') || error.message.includes('network error')) {
//         errorMessage = 'Cannot connect to payment server. Please check if the server is running.';
//         setServerStatus('offline');
//       } else if (error.message.includes('404')) {
//         errorMessage = 'The payment API endpoint could not be found. This functionality may not be implemented yet.';
//       }
          ...
          vendorOrdersData.forEach(order => {
            if (order.items && order.items.length > 0) {
              order.items.forEach(item => {
                ...
                // KEY CHANGE: Always create a new item with order-specific information
                allItems.push({
                  ...item,
                  id: itemId,
