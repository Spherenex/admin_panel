...
const OrderManagement = () => {
  ...
  const autoAssignVendorDirectly = async (orderId, orderData) => {
    try {
      ...
      if (!customerAddress) {
        ...
        await transitionToManualAssignmentDirectly(orderId, orderData, []);
        return;
      }
      ...
      if (!nearestVendors || nearestVendors.length === 0) {
        logAutoAssign(`No active vendors found for order ${orderId}, requiring manual assignment`);
  ...
  const transitionToManualAssignmentDirectly = async (orderId, orderData, attempts = []) => {
    try {
      logAutoAssign(`Transitioning order ${orderId} to manual assignment`);
      ...
      setAdminAlerts(prev => [
        ...prev,
        {
          ...
          type: 'warning',
          message: `Order ${orderIdMap[orderId] || orderId} requires manual assignment after ${attempts.length} auto-assignment attempts.`,
          autoClose: false
        }
      ]);
  ...
  useEffect(() => {
    ...
    const checkForVendorTimeouts = async () => {
      ...
      // Query for orders awaiting vendor confirmation
      const pendingConfirmationRef = query(
        ref(db, 'orders'),
        orderByChild('status'),
        equalTo('pending_vendor_confirmation')
      );

      try {
        const snapshot = await get(pendingConfirmationRef);

        if (!snapshot.exists()) {
          return; // No orders awaiting confirmation
        }
  ...
  const processNextVendorDirectly = async (orderId, orderData) => {
    try {
      ...
      if (!customerAddress) {
        ...
        await transitionToManualAssignmentDirectly(orderId, orderData, assignmentAttempts);
        return;
      }
      ...
      const availableVendors = allVendors.filter(vendor => !triedVendorIds.includes(vendor.id));

      logAutoAssign(`Found ${availableVendors.length} untried vendors for order ${orderId}`);
  ...
  const transitionToManualAssignment = async (orderId, attempts = []) => {
    try {
      const order = orders.find(o => o.id === orderId);
      if (!order) return;

      console.log(`Transitioning order ${orderId} to require manual assignment after ${attempts.length} auto-assignment attempts`);
  ...
  const processNextVendor = async (orderId) => {
    try {
      ...
      if (!customerAddress) {
        logAutoAssign(`No customer address found for order ${orderId}`);
        await transitionToManualAssignment(orderId, assignmentAttempts);
        return;
      }
      ...
      // If there's an error, transition to manual assignment as a fallback
      try {
        await transitionToManualAssignment(orderId, []);
      } catch (err2) {
        console.error('Error transitioning to manual assignment after reassignment failure:', err2);
      }
    }
  ...
  const autoAssignVendor = async (orderId) => {
    try {
      ...
      if (nearestVendors.length === 0) {
        logAutoAssign('No active vendors found for auto-assignment');
  ...
  const VendorCellContent = ({ order }) => {
    ...
    if (order.status === 'pending_manual_assignment') {
      return (
        <button
          className="assign-vendor-button small urgent"
          onClick={() => openAssignVendorModal(order.id)}
        >
