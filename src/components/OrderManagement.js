...
//     // Check if vendor supports at least one of the required categories
...
//       if (vendorCategories[category] === true || 
...
//       // First try: Find vendors within threshold that support the order categories
//       let eligibleVendors = vendorsWithDistance.filter(vendor => 
//         parseFloat(vendor.distance) <= NEARBY_VENDOR_THRESHOLD_KM && 
//         vendorSupportsCategories(vendor, orderCategories)
//       );

//       logAutoAssign(`Found ${eligibleVendors.length} nearby vendors within ${NEARBY_VENDOR_THRESHOLD_KM}km that support the required categories`);

//       // If no nearby vendors with matching categories, try any vendor with matching categories regardless of distance
//       if (eligibleVendors.length === 0 && orderCategories.length > 0) {
//         logAutoAssign(`No nearby vendors with matching categories, searching for any vendor with matching categories`);
//         eligibleVendors = vendorsWithDistance.filter(vendor => 
//           vendorSupportsCategories(vendor, orderCategories)
//         );
//         logAutoAssign(`Found ${eligibleVendors.length} vendors with matching categories regardless of distance`);
//       }

//       // If still no eligible vendors, fall back to any nearby vendor
...
//       // First try: Find untried vendors within threshold that support the order categories
...
//       logAutoAssign(`Found ${availableVendors.length} untried nearby vendors within ${NEARBY_VENDOR_THRESHOLD_KM}km that support the required categories`);

//       // If no nearby untried vendors with matching categories, try any untried vendor with matching categories regardless of distance
//       if (availableVendors.length === 0 && orderCategories.length > 0) {
//         logAutoAssign(`No nearby untried vendors with matching categories, searching for any untried vendor with matching categories`);
//         availableVendors = vendorsWithDistance.filter(vendor => 
//           !triedVendorIds.includes(vendor.id) && 
//           vendorSupportsCategories(vendor, orderCategories)
//         );
//         logAutoAssign(`Found ${availableVendors.length} untried vendors with matching categories regardless of distance`);
//       }
